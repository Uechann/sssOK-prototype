...
- **Firebase 모드** (값이 채워져 있을 때) — 방·폴더·멤버 메타데이터는 Firestore(`sssok_rooms`
  컬렉션)에, 사진·영상 원본은 Firebase Storage(`sssok/{code}/{photoId}/{파일명}`)에 저장합니다.
  `onSnapshot`으로 실시간 구독하므로 **다른 기기·다른 브라우저에서도** 같은 방이 그대로 보입니다.
...
사진·영상 업로드 모두 Storage를 쓰므로 아래 세 가지가 다 필요합니다.

1. **Firestore 규칙** — 콘솔 → Firestore Database → 규칙 탭에 [`firestore.rules`](firestore.rules)
   붙여넣고 게시. (share-drop과 같은 프로젝트를 재사용하는 경우 기존 규칙에 `sssok_rooms` 블록만
   추가된 형태라 그대로 덮어써도 share-drop 쪽 규칙은 유지됩니다)
2. **Storage 규칙** — 콘솔 → Storage → (버킷이 없다면 "시작하기"로 생성) → 규칙 탭에
   [`storage.rules`](storage.rules) 붙여넣고 게시.
   격자용 썸네일이 `{photoId}/thumb/preview.jpg`로 한 단 더 들어가므로 재귀 와일드카드
   (`{fileName=**}`)가 필요합니다 — 예전 단일 세그먼트 규칙이 걸려 있으면 썸네일 업로드가
   권한 거부로 실패합니다
3. **버킷 CORS** — 브라우저에서 곧장 Storage로 업로드/다운로드하기 때문에 필요합니다.
   [Google Cloud Console → Cloud Storage → 버킷](https://console.cloud.google.com/storage/browser)에서
   해당 버킷 → "구성" 탭 → CORS 편집으로 이 저장소의 [`cors.json`](cors.json) 내용을 적용하거나,
...
이 단계를 건너뛰면 업로드가 실패로 뜹니다. 실제로 겪은 문제와 해결 과정은
[`troubleshooting/`](troubleshooting/)에 정리해뒀습니다.
...
- **계측 대시보드** — `#/admin`에서 유저 테스트용 이벤트 로그를 볼 수 있습니다 (아래 참고).
- **프로토타입 시나리오** — 방 화면 왼쪽 아래 `시나리오` 칩으로 업로드/다운로드 실패,
  만료 임박·즉시 만료를 강제로 만들어 실제 조건 없이 실패·만료 화면을 확인할 수 있습니다.

## 계측과 대시보드

앱 곳곳에 "이 일이 일어났다"는 이벤트를 남기고 `#/admin`에서 모아 봅니다(유저 테스트 회고용,
사용자 동선에는 없는 개발자 전용 화면). 호스트/게스트 퍼널, 화면별 체류시간·이탈, 실패 지점,
유입 경로별 방 도달, 세션 타임라인, "하려다 못 한 것"(연 뒤 포기한 시트, 아무 반응 없는 곳을
누른 기록 등)을 보여줍니다. 개인 식별 정보(닉네임·파일명·사진)는 남기지 않고 방 코드도 해시로만
저장합니다(`lib/analytics.ts`). 저장은 `sssok_events` 컬렉션에 한 건씩(수정·삭제 불가) — 방 문서와
분리해 무거운 실시간 구독에 이벤트까지 얹지 않도록 했습니다. 자세한 설계는 `screens/Admin.tsx`,
`lib/adminStats.ts` 코드에 있습니다.
...
- **원본 저장** — 사진·영상 모두 재인코딩 없이 원본 그대로 Storage에 올립니다.
  다운로드도 이 원본을 그대로 받습니다 (`lib/media.ts`)
- **격자용 썸네일** — 원본과 별도로 480px JPEG 축소본을 만들어
  `sssok/{code}/{photoId}/thumb/preview.jpg`에 함께 올립니다. 갤러리 타일은 이걸 보므로
  방을 열 때 원본을 통째로 받지 않습니다. 영구 캐시(`immutable`)라 재방문 시 재요청이 없습니다.
  화면에 보이는 동작은 그대로고, 받는 바이트만 줄어듭니다
- **한 장씩 반영** — 배치 전체를 기다리지 않고 파일 하나가 끝날 때마다 격자에 나타납니다.
  격자에 보이는 사진과 진행 바의 개수가 항상 일치합니다 (`onPhotoUploaded`)
...
- Firebase 모드에서 업로드는 실제 `uploadBytesResumable` 진행률입니다(Storage 설정이 안 돼
  있으면 실패). 로컬 모드는 연출입니다. 다운로드/실패 연출도 모든 모드에서 시뮬레이션입니다
